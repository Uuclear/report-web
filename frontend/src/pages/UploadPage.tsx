  ...
  const uploadFiles = useCallback(async () => {
    ...
    try {
      for (const fileInfo of files) {
        ...
        formData.append('source_type', 'auto') // 自动识别数据来源
    ...
    <div className="space-y-4">
      ...
      >
        ...
        <div className="text-center">
          ...
          <p className="text-base text-gray-600 mb-1">拖拽文件到此处，或点击选择文件</p>
          <p className="text-sm text-gray-400">支持批量上传 JPG、PNG、PDF 格式，自动识别数据来源</p>
        </div>
      </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
          <div className="px-4 py-3 border-b border-gray-200 flex items-center justify-between">
            ...
            <div className="flex gap-2">
              <button onClick={clearAll} className="px-3 py-1.5 text-sm text-gray-600 hover:bg-gray-100 rounded-lg flex items-center gap-1">
              ...
              {pendingFiles.length > 0 && (
                <button
                  ...
                  className="px-4 py-1.5 text-sm bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 font-medium disabled:opacity-50 flex items-center gap-1"
                >
                ...
                <div className="px-4 py-3 flex items-center gap-3">
                  ...
                  <div className="flex items-center gap-2">
                    {fileItem.status === 'pending' && (
                      <span className="px-2 py-0.5 bg-gray-100 text-gray-600 rounded-full text-xs font-medium">待上传</span>
                    )}
                  ...
                  <div className="px-4 py-3 bg-gray-50 border-t border-gray-100">
                    <div className="flex items-center flex-wrap gap-2 mb-2">
                      ...
                      {fileItem.data.method === 'ai_ocr' && (
                        <span className="px-2 py-0.5 bg-amber-100 text-amber-900 rounded-full text-xs font-medium">
                          AI 识别
                        </span>
                      )}
      ...
      {files.length > 0 && (completedFiles.length > 0 || failedFiles.length > 0) && (
        <div className="grid grid-cols-3 gap-3">
          ...
          <div className="bg-green-50 rounded-xl border border-green-200 p-3 text-center">
            <p className="text-xl font-bold text-green-700">{completedFiles.length}</p>
            <p className="text-xs text-green-600">处理成功</p>
          </div>
          <div className="bg-red-50 rounded-xl border border-red-200 p-3 text-center">
            <p className="text-xl font-bold text-red-700">{failedFiles.length}</p>
            <p className="text-xs text-red-600">处理失败</p>
          </div>
        </div>
      )}
    </div>
