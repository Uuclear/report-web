"""
微信二维码扫描器 - 使用 wechat_qrcode 支持图片和PDF文件
...
class WeChatQRCodeScanner:
    ...
    def __init__(self, model_dir: str = "d:/code/trytry/models"):
        """
        初始化微信二维码扫描器
    ...
    def scan_image(self, image_path: str) -> Tuple[List[str], List[List]]:
        """
        扫描图片中的二维码
        ...
        # 转换结果
        decoded_texts = list(results) if results else []
        points_list = [p.tolist() if hasattr(p, 'tolist') else list(p) for p in points] if points else []
        
        return decoded_texts, points_list
    
    def scan_pdf(self, pdf_path: str) -> List[Tuple[int, List[str], List[List]]]:
        """
        扫描PDF文件中的二维码
        ...
        for page_num, img_pil in enumerate(images, start=1):
            # PIL Image 转 numpy array (BGR格式,cv2需要)
            img_bgr = cv2.cvtColor(np.array(img_pil), cv2.COLOR_RGB2BGR)
            
            # 检测和解码
            decoded_texts, points_list = self.scan_image_from_array(img_bgr)
            
            results.append((page_num, decoded_texts, points_list))
        
        return results
    
    def scan_image_from_array(self, img_bgr: np.ndarray) -> Tuple[List[str], List[List]]:
        """
        从numpy数组扫描二维码
        ...
        Returns:
            (解码结果列表, 点位信息列表)
        """
        results, points = self.detector.detectAndDecode(img_bgr)
        decoded_texts = list(results) if results else []
        points_list = [p.tolist() if hasattr(p, 'tolist') else list(p) for p in points] if points else []
        return decoded_texts, points_list
    ...
    def scan_file(self, file_path: str) -> dict:
        ...
        try:
            if ext == self.pdf_extension:
                ...
                for page_num, decoded_texts, points_list in pages:
                    for i, text in enumerate(decoded_texts):
                        if text:  # 只保存成功解码的
                            # points_list[i] 包含4个角点坐标
                            bbox = points_list[i] if i < len(points_list) else []
                            result['qrcodes'].append({
                                'page': page_num,
                                'content': text,
                                'confidence': None,  # wechat_qrcode不提供置信度
                                'bbox': bbox
                            })
                ...
                # 图片文件
                result['type'] = 'image'
                decoded_texts, points_list = self.scan_image(str(file_path))
...
def main():
    """主函数"""
    # 初始化扫描器
    scanner = WeChatQRCodeScanner(model_dir="d:/code/trytry/models")
    ...
    # 保存完整结果到JSON
    output_file = r"d:\code\trytry\results\wechat_qrcode_results.json"
